...
namespace UnityEditor.Tilemaps
{
    ...
    public class PrefabBrush : BasePrefabBrush
    {
        ...
        public override void Rotate(RotationDirection direction, GridLayout.CellLayout layout)
        {
            ...
            /*Transform[] allChildren = MapManager.GameObject.GetComponentsInChildren<Transform>();
            for (int i = 0; i < allChildren.Length; i++)
            {
                Transform Object = allChildren[i];
                BlockHide BlockHide = Object.GetComponent<BlockHide>();
                if (BlockHide != null)
                    BlockHide.BlockRefresh();
            }*/
        }
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            ...
            bool physics = Physics.Raycast(new Vector3(position.x + (MapManager.Grid.cellGap.x + 1) * 0.5f, position.y + (MapManager.Grid.cellGap.y + 1) * 0.5f, position.z + (MapManager.Grid.cellGap.z + 1) * 0.5f), new Vector3(-1, 0, 0), out raycastHit, 1f, LayerMask.GetMask("Map", "Water", "Glass"));
            if (physics)
            {
                BlockHide blockHide = raycastHit.collider.gameObject.GetComponent<BlockHide>();
                blockHide.BlockRefresh();
            }

            physics = Physics.Raycast(new Vector3(position.x + (MapManager.Grid.cellGap.x + 1) * 0.5f, position.y + (MapManager.Grid.cellGap.y + 1) * 0.5f, position.z + (MapManager.Grid.cellGap.z + 1) * 0.5f), new Vector3(1, 0, 0), out raycastHit, 1f, LayerMask.GetMask("Map", "Water", "Glass"));
            if (physics)
            {
                BlockHide blockHide = raycastHit.collider.gameObject.GetComponent<BlockHide>();
                blockHide.BlockRefresh();
            ...
            physics = Physics.Raycast(new Vector3(position.x + (MapManager.Grid.cellGap.x + 1) * 0.5f, position.y + (MapManager.Grid.cellGap.y + 1) * 0.5f, position.z + (MapManager.Grid.cellGap.z + 1) * 0.5f), new Vector3(0, -1, 0), out raycastHit, 1f, LayerMask.GetMask("Map", "Water", "Glass"));
            if (physics)
            {
                BlockHide blockHide = raycastHit.collider.gameObject.GetComponent<BlockHide>();
                blockHide.BlockRefresh();
            ...
            physics = Physics.Raycast(new Vector3(position.x + (MapManager.Grid.cellGap.x + 1) * 0.5f, position.y + (MapManager.Grid.cellGap.y + 1) * 0.5f, position.z + (MapManager.Grid.cellGap.z + 1) * 0.5f), new Vector3(0, 0, -1), out raycastHit, 1f, LayerMask.GetMask("Map", "Water", "Glass"));
            if (physics)
            {
                BlockHide blockHide = raycastHit.collider.gameObject.GetComponent<BlockHide>();
                blockHide.BlockRefresh();
        ...
        {
            System.Collections.Generic.List<GameObject> list = GetObjectsInCell(grid, brushTarget.transform, position);
            for (int i = 0; i < list.Count; i++)
            {
                GameObject objectInCell = list[i];
        ...
        public override void Pick(GridLayout gridLayout, GameObject brushTarget, BoundsInt position, Vector3Int pickStart)
        {
            ...
            System.Collections.Generic.List<GameObject> list = GetObjectsInCell(gridLayout, brushTarget.transform, position.position);
            for (int i = 0; i < list.Count; i++)
            {
                GameObject objectInCell = list[i];
                if (objectInCell)
            ...
            /*Transform[] allChildren = MapManager.GameObject.GetComponentsInChildren<Transform>();
            foreach (Transform Object in allChildren)
            {
                BlockHide BlockHide = Object.GetComponent<BlockHide>();
                if (BlockHide != null)
                    BlockHide.BlockRefresh();
